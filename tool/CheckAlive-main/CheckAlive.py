...
def get_parser():
    ...
    p.add_argument("-x", "--xcode", type=str,
                   help="要排除的响应码，如-x 404，即状态码为404也判定为不存活，多个参数用逗号分隔")
    p.add_argument("-a", "--addcode", type=str,
                   help="要增加的响应码，如-a 500，即状态码为500的也判定为存活，多个参数用逗号分隔")
...
def check_urls(file, proxy, xcode, addcode):
    ...
    try:
        urls = [url.strip() if url.startswith("http://") or url.startswith("https://") else "http://" + url.strip() for
                url in open(file, "r", encoding="utf-8")]
            ...
            while retry_count > 0:
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.get(url, headers=headers, ssl=False, timeout=18, allow_redirects=False,
