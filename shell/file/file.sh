...
# 保留7天
tmp1data=( "${pwd}/tmp/master" "${pwd}/tmp/tmp" )

# 保留三天
tmpdata=( "/${pwd}/tmp/merge" "${pwd}/tmp/npgMerge" "${pwd}/tmp/share" "${pwd}/tmp2/download" "${pwd}/tmp2/cover" "${pwd}/tmp2/vlog" )
...
deldirectories() {
    for file in $tmp1data; do
      find $file/*  -maxdepth 3 -type d -ctime +7 -exec rm -rf {} \; -printf "Deleting: %p\t%TY-%Tm-%Td %TH:%TM:%.2TS\n" >> $logfile
    done
    for file in $tmpdata; do
      find $file/* -maxdepth 3 -type d -ctime +3 -exec rm -rf {} \; -printf "Deleting: %p\t%TY-%Tm-%Td %TH:%TM:%.2TS\n" >> $logfile
    done
}
